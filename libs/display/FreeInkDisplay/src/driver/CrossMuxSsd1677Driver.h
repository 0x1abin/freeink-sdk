...
// CrossMuxSsd1677Config. A board reuses the generic driver by passing its own config;
...
enum class CrossMuxSsd1677CleanPolicy : uint8_t { Legacy, BlackPulse };

// Device-tunable SSD1677 waveform/config. A board overrides only what differs.
struct CrossMuxSsd1677Config {
  ...
  CrossMuxSsd1677CleanPolicy cleanPolicy = CrossMuxSsd1677CleanPolicy::Legacy;
};
...
class CrossMuxSsd1677Driver : public PanelDriver {
 public:
  explicit CrossMuxSsd1677Driver(const CrossMuxSsd1677Config& cfg = crossmuxSsd1677DefaultConfig());
...
#if FREEINK_DEVICE_MURPHY_M4
PanelDriver& crossmuxSsd1677MurphyM4Driver(MurphyM4Batch batch);
#endif
