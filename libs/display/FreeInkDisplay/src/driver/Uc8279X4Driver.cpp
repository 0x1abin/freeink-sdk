...
const uint8_t (*scaledQualityBank())[GRAY_LUT_LEN] {
  ...
  // Same bounded fields as the wire format; 140 bytes of scratch, no heap.
  static_assert(sizeof(Phase) == 1);
...
bool Uc8279X4Driver::displayStart(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, RefreshMode mode, bool turnOff,
  ...
  if (!fast) {
    ...
    // Re-drive every pixel for inverted content or once after grayscale,
    // preventing dark-background residue and scrubbing residual AA edge charge.
    streamPlane(bus, CMD_DTM1, fb, /*invert=*/true);
  }
