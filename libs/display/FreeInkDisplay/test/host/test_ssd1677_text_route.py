...
class TextRoutingTest(unittest.TestCase):
                ...
                with self.subTest(board=board, enabled=enabled):
                    result = subprocess.run(
                        ["c++", "-std=c++17", "-E", "-dM", "-x", "c++", "-",
                         ...
                         f"-DFREEINK_SSD1677_COMBINED_AA={enabled}"],
    ...
    def test_board_matrix(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for device in ("STICKY", "X4PRO", "X4CLASSIC", "MURPHY_M4", "WAVESHARE_EPAPER_397", "METALIO_EINK4", "PAPERMONO"):
                with self.subTest(device=device):
                    ...
                    command = ["c++", "-std=c++17", "-DARDUINO=1", "-DEINK_DISPLAY_SINGLE_BUFFER_MODE=1",
                               f"-DFREEINK_SSD1677_TEXT_ROUTING={int(device != 'PAPERMONO')}", "-DFREEINK_DRIVER_SSD1677=1",
