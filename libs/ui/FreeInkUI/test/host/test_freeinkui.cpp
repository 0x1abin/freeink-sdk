...
void testDisplayTarget() {
  ...
  // A solid white foreground must not be flipped back to black by the target.
  white.inverted = true;
  white = textStyleWithForeground(white, Paint::solid(Color::White));
...
void testListNavScrollsClippedListWithinRowEstimate() {
  ...
  const auto buildCount = [&](const uint16_t count, const int16_t wantIndex) {
    ...
    props.rowHeight = 20;
    props.labelText.maxLines = 2;
    props.selectionCoversScrollReservation = fullWidthSelection ? 1 : 0;
  ...
  // The fork's full-width selection must not erase the measured scrollbar.
  fullWidthSelection = true;
  build(0);
  CHECK_EQ(scrollFills(), 2);
  CHECK(hasFill(Rect{0, 0, 480, 32}));
  CHECK_EQ(draw.ops[draw.opCount - 2].rect.x, 477);
  CHECK_EQ(draw.ops[draw.opCount - 1].rect.height, 150);
...
void testSelectedTabIndicatorMetrics() {
  class StyledTarget : public FakeDrawTarget {
   public:
    mutable int measureCalls = 0;
    Size measureText(FontId font, const char* text, TextStyle style) const override {
      ++measureCalls;
      return Size{static_cast<int16_t>(std::strlen(text) * (font == 9 && style.bold ? 10 : 6)), 12};
    }
  } draw;
  DeviceContext device = makeDevice();
  InputSnapshot input;
  ...
  bar.selectedText.bold = true;
  tabBar(frame, Rect{0, 0, 300, 40}, bar);
  CHECK_EQ(draw.ops[0].rect.width, 68); // 50px selected label + 10px arrow/gap + 8px padding
  CHECK_EQ(draw.ops[1].kind, FakeDrawTarget::Op::Text);
  CHECK_EQ(draw.ops[1].rect.width, 50);
  CHECK_EQ(draw.ops[1].font, 9);
  CHECK(draw.ops[1].bold);
  CHECK_EQ(draw.ops[1].color, Color::White);
  CHECK_EQ(draw.ops[2].kind, FakeDrawTarget::Op::Triangle);
  CHECK(draw.ops[2].rect.x >= draw.ops[1].rect.right() + bar.indicatorGap);
  CHECK_EQ(draw.measureCalls, 4); // Two labels: width preflight, then drawing.
  draw.measureCalls = 0;
  bar.layout = TabBarLayout::EqualWidth;
  tabBar(frame, Rect{0, 0, 300, 40}, bar);
  CHECK_EQ(draw.measureCalls, 2); // Equal slots need only the drawing pass.
}
...
int main() {
  ...
  testSelectedTabIndicatorMetrics();
