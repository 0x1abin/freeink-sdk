...
class GfxRendererTarget final : public DrawTarget {
 public:
  ...
  void setTextCentering(TextCentering mode) { textCentering_ = mode; }
  ...
  void text(const Rect rect, const char* text, const TextStyle style) override {
    ...
    const auto drawAligned = [&](const char* textLine, const int y) {
      ...
      if (textCentering_ == TextCentering::InkBounds && style.align == TextAlign::Center && textLine[0] >= '0' && textLine[0] <= '9' && textLine[1] == '\0' &&
 ...
 private:
  ...
  TextCentering textCentering_ = TextCentering::InkBounds;
