...
struct ListProps {
  ListLayoutPolicy layoutPolicy = ListLayoutPolicy::Content;
...
inline ListRowLayout measureListRow(const DrawTarget &target, AssetResolver *assets,
  ...
  const TextStyle &labelText = hasState(item.state, StateEmphasized) ? props.emphasizedText : props.labelText;
  const int16_t rowH = props.rowHeight > 0 ? props.rowHeight : 36;
  const int16_t sidePad = props.sidePadding < 0 ? 8 : props.sidePadding;
  const int16_t labelLh = target.lineHeight(labelText.font);
  ...
  if (item.toggle) {
    ...
    if (props.valueMaxWidth > 0 && result.valueWidth > props.valueMaxWidth)
      result.valueWidth = props.valueMaxWidth;
  }
  ...
  const bool measureLabel = props.layoutPolicy != ListLayoutPolicy::ThemeRow || item.subtitle != nullptr ||
                            static_cast<int16_t>(labelLh * labelText.maxLines) > rowH;
  if (measureLabel && item.label && labelText.maxLines > 1 && labelLh > 0 && result.labelWidth > 0) {
  ...
  if (props.rowPaddingY >= 0) {
    ...
    needed = static_cast<int16_t>(result.labelHeight + result.subtitleHeight + (padding > 0 ? padding : 0));
             ...
             (props.layoutPolicy == ListLayoutPolicy::ThemeRow && result.labelLines > 1)) {
...
void list(Frame<MaxInteractions> &frame, Rect rect, const ListProps &props) {
  ...
  for (uint16_t i = top; i < props.count; ++i) {
    ...
    const TextStyle &itemLabelText = hasState(item.state, StateEmphasized) ? props.emphasizedText : props.labelText;
