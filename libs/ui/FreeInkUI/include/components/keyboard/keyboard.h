...
enum class KeyboardGeometry : uint8_t { Separated, Classic };

struct KeyboardProps {
  KeyboardGeometry geometry = KeyboardGeometry::Separated;
...
void keyboard(Frame<MaxInteractions>& frame, Rect rect, const KeyboardProps& props) {
  ...
  auto drawKey = [&](Rect keyRect, const KeyboardKey& key, int16_t selectedIndex) {
    ...
    bp.label = hasAltHint && props.geometry == KeyboardGeometry::Separated ? nullptr : label;
    ...
    if (props.geometry == KeyboardGeometry::Separated) {
    ...
    if (key.kind == KeyKind::Delete || key.kind == KeyKind::Lang) {
      ...
      if (props.geometry == KeyboardGeometry::Classic) {
        ...
        int16_t size = static_cast<int16_t>(((desired + 8) / 16) * 16);
        if (size < 16) size = 16;
        while (size > maxSize && size > 16) size = static_cast<int16_t>(size - 16);
        if (size > maxSize) size = maxSize;
        const BitmapRef icon = key.kind == KeyKind::Delete ? lucideDeleteIcon16() : lucideGlobeIcon32();
        frame.target().bitmap(centeredRect(keyRect, Size{size, size}), icon, BitmapMode::Contain, ink);
        return;
      }
    ...
    if (hasAltHint && props.geometry == KeyboardGeometry::Classic) {
      ...
      frame.target().text(Rect{static_cast<int16_t>(keyRect.x + 2), static_cast<int16_t>(keyRect.y + 2),
                              static_cast<int16_t>(keyRect.width - 3), frame.target().lineHeight(altStyle.font)},
                          key.alt, altStyle);
      return;
    }
    if (hasAltHint) {
