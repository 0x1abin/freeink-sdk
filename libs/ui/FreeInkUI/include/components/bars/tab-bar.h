...
void tabBar(Frame<MaxInteractions>& frame, Rect rect, const TabBarProps& props) {
  ...
  const auto textFor = [&](const TabItem& tab) -> const TextStyle& {
    return tab.selected && !textStyleUnset(props.selectedText) ? props.selectedText : props.text;
  };
  const auto measureContent = [&](const TabItem& tab) {
    TabContentMetrics metrics{};
    const TextStyle& text = textFor(tab);
    metrics.labelWidth = tab.label && tab.label[0] != '\0'
                             ? frame.target().measureText(text.font, tab.label, text).width
                             : 0;
  ...
  const auto contentSlotWidth = [&](const TabContentMetrics& metrics) {
  ...
  if (contentWidthLayout) {
    for (uint8_t i = 0; i < props.count; ++i) {
      naturalWidth += contentSlotWidth(measureContent(props.tabs[i]));
    }
  ...
  if (distribute) {
    if (distributedWidth <= 0) {
      for (uint8_t i = 0; i < props.count; ++i) {
        const int16_t width = contentSlotWidth(measureContent(props.tabs[i]));
  ...
  for (uint8_t i = 0; i < props.count; ++i) {
    ...
    const int16_t slotW = distribute ? distributedWidth : (contentWidthLayout ? contentSlotWidth(metrics) : equalSlotW);
    ...
    if (hasIcon) {
      ...
      if (hasLabel) {
        const int16_t labelH = frame.target().lineHeight(textFor(tab).font);
    ...
    if (hasLabel || metrics.indicatorWidth > 0) {
      TextStyle label = textStyleWithForeground(textFor(tab), style.foreground);
