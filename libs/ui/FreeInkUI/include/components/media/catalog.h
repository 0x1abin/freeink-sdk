...
void coverShelf(Frame<N>& frame, Rect rect, const CoverShelfProps& props) {
  ...
  Rect headerRect = take(body, headerH, props.headerGap);
  const int16_t gap = nonnegative(props.gap);
  ...
  auto control = [&](ButtonProps buttonProps, int16_t desiredWidth, bool available) {
    if (!hasButton(buttonProps) || headerRect.empty()) return;
    const int16_t w = min(headerRect.width, desiredWidth > buttonProps.minTouchSize ? desiredWidth : buttonProps.minTouchSize);
    if (w <= 0) return;
    Rect band{static_cast<int16_t>(headerRect.right() - w), headerRect.y, w, headerRect.height};
    headerRect.width -= min(headerRect.width, static_cast<int16_t>(w + gap));
  ...
  heading.bold = true;
  text(frame.target(), headerRect, props.title, heading, 1, 0);
