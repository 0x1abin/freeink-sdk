...
void publicationPage(Frame<N>& frame, Rect rect, const PublicationPageProps& props) {
  ...
  availabilityH = std::accumulate(details, details + 3, availabilityH, [&](int16_t height, const char* label) {
    return static_cast<int16_t>(height + (present(label) ? frame.target().lineHeight(av.detailText.font) + nonnegative(av.gap) : 0));
  });
