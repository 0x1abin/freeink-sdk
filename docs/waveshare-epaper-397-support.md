...
This guide describes the upstream SDK-managed profile. CrossMux continues to
use the separate `FREEINK_DEVICE_WAVESHARE_EPAPER_397` profile with function-key
gestures, audio, and its `Waveshare397Power` HAL. Do not alias the device flags
or combine the two PMIC owners. The hardware observations below belong to the
upstream profile and are not acceptance results for the CrossMux configuration.
