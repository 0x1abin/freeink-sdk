...
This fork retains two Waveshare 3.97 entrypoints. CrossMux uses
`FREEINK_DEVICE_WAVESHARE_EPAPER_397` with function-key gestures, audio, and
consumer HAL PMIC management. The upstream `FREEINK_DEVICE_WS397` profile uses
SDK PMIC management and BOOT-based sleep/wake. These flags are independent; do
not alias them or enable both PMIC owners for the same board.
